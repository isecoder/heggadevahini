...
const EnvironmentAndWildlife = () => {
  ...
  useEffect(() => {
    const fetchEnvironmentAndWildlifeNews = async () => {
      try {
        ...
        if (data.success && data.data.length > 0) {
          // Filter news containing the "Environment and Wildlife" tag
          const environmentAndWildlifeNews = data.data
            .filter((news: NewsItem) =>
              news.tags.includes("Environment and Wildlife")
            ) // Change tag to "Environment and Wildlife"
            .sort(
              (a: NewsItem, b: NewsItem) =>
                new Date(b.createdAt).getTime() -
                new Date(a.createdAt).getTime()
            )
            .slice(0, 3); // Get the latest 3 environment and wildlife news articles

          if (environmentAndWildlifeNews.length === 0)
            throw new Error("No 'Environment and Wildlife' news available.");

          setNewsList(environmentAndWildlifeNews);
        } else {
    ...
    fetchEnvironmentAndWildlifeNews();
  }, []);
  ...
  if (loading) return <p>Loading environment and wildlife news...</p>;
  if (error) return <p className="text-red-500">{error}</p>;
  if (newsList.length === 0) return <p>No environment and wildlife news available.</p>;
  ...
  return (
    <section className="bg-[#FFF3E9] p-4 rounded-lg">
      <div className="relative group">
        ...
        {/* Tags (Filtered to Only Show "Environment and Wildlife") */}
        <div className="mt-3 flex gap-2">
          {news.tags
            .filter((tag: string) => tag === "Environment and Wildlife") // Change 'Politics' to 'Environment and Wildlife'
            .map(
              (
                tag: string,
...
export default EnvironmentAndWildlife;
