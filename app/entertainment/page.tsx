...
const TrendingNews = () => {
  ...
  const fetchNews = async () => {
    ...
    try {
      ...
      const trendingNews = data.data
        .filter((item: NewsItem) => item.tags.includes("Entertainment"))
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <h1 className="text-3xl font-bold text-blue-600 text-center mb-6">ಮನರಂಜನೆ ಸುದ್ದಿ</h1>
      {loading ? (
        <LoadingSpinner />
      ) : news.length === 0 ? (
        <p className="text-center text-gray-600">ಯಾವುದೇ ಮನರಂಜನೆ ಸುದ್ದಿ ಲಭ್ಯವಿಲ್ಲ.</p>
      ) : (
