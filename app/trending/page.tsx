...
const TrendingNews = () => {
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <h1 className="text-3xl font-bold text-blue-600 text-center mb-6">🔥 ಟ್ರೆಂಡಿಂಗ್ ಸುದ್ದಿ</h1>
      {loading ? (
        <LoadingSpinner />
      ) : news.length === 0 ? (
        <p className="text-center text-gray-600">ಯಾವುದೇ ಟ್ರೆಂಡಿಂಗ್ ಸುದ್ದಿ ಲಭ್ಯವಿಲ್ಲ.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {news.map((item) => {
            ...
            return (
              ...
              >
                <Image src={imageUrl} alt={item.title || "ಸುದ್ದಿ ಚಿತ್ರ"} width={400} height={250} className="rounded-md" />
                <h2 className="text-xl font-semibold mt-3">{item .title || "ಅನಾಮಧೇಯ ಸುದ್ದಿ"}</h2>
                <p className="text-gray-700 mt-2 line-clamp-3">{item .content || "ವಿವರಣೆ ಲಭ್ಯವಿಲ್ಲ."}</p>
